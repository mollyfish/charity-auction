...
authRouter.get('/signin', basicHttp, function(req, res) {
  if (!(req.auth.username && req.auth.password)) {
    console.log('no basic auth provided: no username and no password');
    return res.status(401).json({msg: 'no username and no password'});
  }

  User.findOne({'auth.basic.username': req.auth.username}, function(err, user) {
    if (err) {
      console.log('no basic auth provided: username not found');
      return res.status(401).json({msg: 'username not found'});
    }

    if (!user) {
      console.log('no basic auth provided: no user');
      return res.status(401).json({msg: 'no user'});
    }

    if (!user.checkPassword(req.auth.password)) {
     console.log('no basic auth provided: wrong password');
